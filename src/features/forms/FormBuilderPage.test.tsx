...
describe('FormBuilderPage',()=>{it('shows immutable published schema and version action',async()=>{vi.mocked(formApi.list).mockResolvedValue([published]);vi.mocked(formApi.get).mockResolvedValue(published);vi.mocked(documentApi.adminRequirements).mockResolvedValue([]);renderPage();expect(await screen.findByRole('heading',{name:'Başvuru formu builder'})).toBeInTheDocument();expect(await screen.findByText('Aile geliri *')).toBeInTheDocument();expect(screen.getByRole('button',{name:'Yeni versiyon oluştur'})).toBeEnabled()})})
