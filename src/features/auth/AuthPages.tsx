...
export function LoginPage() {
  ...
  return (
    <AuthCard title="Oturum aç">
      ...
      <p>
        <Link to="/forgot-password">Şifremi unuttum</Link> · <Link to="/register">Kayıt ol</Link>
      </p>
    </AuthCard>
  );
}
...
export function ForgotPasswordPage() {
  return <EmailAction title="Şifremi unuttum" action={authApi.forgotPassword} />;
}
export function ResendVerificationPage() {
  return (
    <EmailAction title="Doğrulama e-postasını yeniden gönder" action={authApi.resendVerification} />
  );
}
...
function EmailAction({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await authApi.csrf();
    await action(String(new FormData(event.currentTarget).get('email')));
    setMessage('E-posta adresi uygunsa bağlantı gönderildi.');
  }
  return (
...
export function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .then(() => setMessage('E-posta doğrulandı. Artık giriş yapabilirsin.'))
...
export function ResetPasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await authApi.csrf();
    await authApi.resetPassword(token, String(new FormData(event.currentTarget).get('password')));
    setMessage('Şifren yenilendi.');
  }
  return (
    <AuthCard title="Yeni şifre">
      <form onSubmit={submit}>
        <Field name="password" label="Yeni şifre" type="password" minLength={10} />
        {message && <p className="status status--success">{message}</p>}
        <button disabled={!token}>Şifreyi yenile</button>
      </form>
    </AuthCard>
  );
}
