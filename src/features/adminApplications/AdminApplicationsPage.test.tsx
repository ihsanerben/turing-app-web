...
vi.mock('./adminApplicationApi', () => ({
  adminApplicationApi: { list: vi.fn(), detail: vi.fn(), addNote: vi.fn(), changeStatus: vi.fn() },
}));
...
describe('AdminApplicationsPage', () => {
  it('renders paged applications and opens internal detail', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Başvuru yönetimi' })).toBeInTheDocument();
    fireEvent.click(await screen.findByRole('button', { name: /Ada Lovelace/ }));
    expect(
      await screen.findByText(
        (_, element) =>
          element?.tagName === 'LI' && element.textContent?.includes('Kontrol edildi.') === true,
      ),
    ).toBeInTheDocument();
