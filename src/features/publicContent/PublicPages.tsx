...
import { publicContentApi, type Faq } from './publicContentApi';
...
export function HomePage() {
  return (
    <section className="public-page hero">
      ...
      <h1>Potansiyelini geleceğe taşı.</h1>
      <p>
        Burs programlarını incele, başvuru dönemlerini takip et ve eğitim yolculuğunda yanında
        olacak desteğe ulaş.
      </p>
...
export function ScholarshipsPage() {
  ...
  return (
    <PublicList title="Burs programları" error={error}>
      {data?.map((v) => (
...
export function ScholarshipDetailPage() {
  ...
  const { data, error } = useLoad(load);
  return (
    <PublicList title={data?.program.name ?? 'Burs programı'} error={error}>
      {data && (
        <>
...
export function FaqPage() {
  const load = useCallback(() => publicContentApi.faqs(), []);
  const { data, error } = useLoad(load);
  return (
    <PublicList title="Sıkça sorulan sorular" error={error}>
      {data?.map((v: Faq) => (
        <details key={v.id}>
          <summary>{v.question}</summary>
          <p>{v.answer}</p>
        </details>
      ))}
    </PublicList>
...
export function AboutPage() {
  return (
    <PublicList title="Hakkımızda">
      <p>
        Turing Scholarship, öğrencilerin eğitim yolculuklarını şeffaf ve erişilebilir burs
        süreçleriyle desteklemek için çalışır.
      </p>
    </PublicList>
  );
}
export function ContactPage() {
  return (
    <PublicList title="İletişim">
      <p>
        Burs süreçleriyle ilgili sorularınız için{' '}
        <a href="mailto:info@turing.local">info@turing.local</a> adresinden bize ulaşabilirsiniz.
      </p>
    </PublicList>
  );
}
...
function format(v: string) {
  return new Intl.DateTimeFormat('tr-TR', { dateStyle: 'medium' }).format(new Date(v));
}
