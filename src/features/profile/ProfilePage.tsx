...
export function ProfilePage() {
  ...
  useEffect(() => {
    if (profile?.universityId) profileApi.departments(profile.universityId).then(setDepartments);
    ...
    try {
      ...
      setError(
        axios.isAxiosError(reason)
          ? (reason.response?.data?.message ?? 'Profil kaydedilemedi.')
  ...
  return (
    <section className="card profile-card">
      ...
      <h1>Profilim</h1>
      <p>Buradaki bilgiler sonraki başvurularında tekrar kullanılacak.</p>
        ...
        <fieldset>
          ...
          <label>
            ...
            <select
              ...
              onChange={(e) => {
                change('universityId', e.target.value || null);
                change('departmentId', null);
                setDepartments([]);
                if (e.target.value) change('otherUniversity', null);
              }}
            >
              <option value="">Listede yok / seçilmedi</option>
              {universities.map((u) => (
          ...
          {!profile.universityId && (
            <Field
              label="Diğer üniversite"
              value={profile.otherUniversity}
              onChange={(v) => change('otherUniversity', v)}
            />
          )}
          {profile.universityId && (
            <label>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value) change('otherDepartment', null);
                }}
              >
                <option value="">Listede yok / seçilmedi</option>
                {departments.map((d) => (
          ...
          {!profile.departmentId && (
            <Field
              label="Diğer bölüm"
              value={profile.otherDepartment}
              onChange={(v) => change('otherDepartment', v)}
            />
          )}
          <label>
            Eğitim seviyesi
            <select
              ...
              onChange={(e) =>
                change('educationLevel', (e.target.value || null) as Profile['educationLevel'])
              }
            >
          ...
          <Field
            ...
            min={1}
            max={8}
            value={profile.studyYear?.toString() ?? null}
            onChange={(v) => change('studyYear', v ? Number(v) : null)}
          />
          <Field
            label="Not ortalaması (4 üzerinden)"
            type="number"
            min={0}
            max={4}
            step="0.01"
            value={profile.gpa?.toString() ?? null}
            onChange={(v) => change('gpa', v ? Number(v) : null)}
          />
        </fieldset>
...
function Field({
  ...
  onChange,
  ...props
}: { label: string; value: string | null; onChange: (value: string | null) => void } & Omit<
